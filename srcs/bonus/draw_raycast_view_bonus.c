...
/*   Updated: 2023/09/02 16:28:42 by jebouche         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_draw_info *d_info)
{
	...
	ray_fix = correct_degrees(scene->player.angle - ray_angle);
	...
	while (d_info->project_y >= d_info->floor_start)
	{
		...
		d_info->tex.x = (scene->player.location.x + cos(deg_to_rad(ray_angle)) 
				* 370 * TEXTURE_SIZE / delta_y / ray_fix);
		d_info->tex.y = (scene->player.location.y - sin(deg_to_rad(ray_angle)) 
				* 370 * TEXTURE_SIZE / delta_y / ray_fix);
		d_info->tex.color = ft_pixel_get(scene->bonus_textures[1], 
				(int)d_info->tex.x & 63, (int)d_info->tex.y & 63);
		ft_pixel_put(cubed->raycast_info->r_img, d_info->project_x, 
			d_info->project_y, d_info->tex.color);
		d_info->tex.color = ft_pixel_get(scene->bonus_textures[2], 
				(int)d_info->tex.x & 63, (int)d_info->tex.y & 63);
		ft_pixel_put(cubed->raycast_info->r_img, d_info->project_x, 
